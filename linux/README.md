...
- Create a Bash Script named `automation.sh` to:
  ...
  - After building your `Dockerfile`, your `Bash Script` is supposed to tag your
    image dinamically;
    - You can use hash, date, sequencial or any tagging approach you think it is
      good here;
  - Use your `script.yaml` as a template file to:
    - Create a new file called `new-app.yaml` replcaing the `MY_NEW_IMAGE` by
      your new image tag/name;
  - Use `kubectl` to show the difference between the current state in your
    `minikube` and your newly created `new-app.yaml`;

## Expected Output

Please, provide us with a file named `automation.sh` you created. Your
